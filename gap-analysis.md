...
| C10 | `TextCrusher` | fn | spec | all | `docs/text-and-logs.mdx` | no | M | Lossless plain-text pass (whitespace, repetition). Done — `TextCrusher` (lossless) and `TextSummarizer` (lossy), split per I10. Registered in `Orchestrator` and reached from the request path via `transform_for_block`, which routes prose only for tool-output blocks (D24). |
